...
ax2.set(xlim=(4e-2,3e-1), ylim=(0, 0.5))
v = np.logspace(1e-20,1e0,nticks)

r = loadtxt("data/rFrac_T_contour.dat",usecols=0)
w = loadtxt("data/w_T_contour.dat",usecols=0)
#dat = loadtxt("data/primakoff_contour_1e2.dat",delimiter='	')
dat = loadtxt("data/primakoff_L_contour_1e2.dat",delimiter='	')
#dat = loadtxt("data/scalarB_contour_1e2.dat",delimiter='	')

#dat = integrand(1e2,w,r,ne,T,rho)
conts = ax2.contourf(w/1e3,r,dat,nticks)#,locator=ticker.LogLocator())

# axes
ax2.set_xlabel("Scalar energy [keV]")
ax2.set_ylabel("Solar radius fraction")	#[m-2 s-1 eV-1]")
ax2.set_xscale('log')
#ax2.set_yscale('log')
fig2.colorbar(conts,label=r'$\beta_\gamma^{-2}\frac{\mathrm{d}^2N}{\mathrm{d}\omega\mathrm{d}r}$ [eV]')#,values=v,boundaries=v)#,ticks=v)

plt.savefig('plots/L_contour--lin-100--reduced2.jpg')
plt.show()
