...
if dplotres==1:
  ...
  plt.plot(xT, mu1, 'r-',label=r'$\beta_m = 10^5')
  plt.plot(xT, mu2, 'b-',label=r'$\beta_m = 10^6')
  plt.plot(xT, mu3, 'g-',label=r'$\beta_m = 10^7')
  plt.xscale('log')
  plt.yscale('log')
  plt.xlabel(r"Solar radius $r/R_\odot$")
  plt.ylabel(r"Non-res integrand")
  plt.xlim((1.e-3, 1.))
  plt.legend()
  plt.show()
  exit()
...
#  mu2 = profnonres(omp, 1.e8, 1.e10, 1)*(RSun/ASun)**2
  ...
  plt.ylabel(r"Flux [cm$^{-2}$ s$^{-1}$ keV$^{-1}$]")
#  plt.plot(omp, mu2, 'b-')
#  plt.xscale('log')
#  plt.yscale('log')
#  plt.xlim((0.01, 30))
